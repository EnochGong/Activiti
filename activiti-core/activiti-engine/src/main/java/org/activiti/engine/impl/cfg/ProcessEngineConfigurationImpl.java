...
public abstract class ProcessEngineConfigurationImpl extends ProcessEngineConfiguration {
  ...
  */
  ...
  public Collection<? extends Deployer> getDefaultDeployers() {
    List<Deployer> defaultDeployers = new ArrayList<Deployer>(); // 初始化defaultDeployers集合，用来存储所有的部署器对象

    if (bpmnDeployer == null) {
      bpmnDeployer = new BpmnDeployer(); // 实例化BpmnDeployer类
    }
  ...
  public void initListenerFactory() {
    if (listenerFactory == null) { // 设置默认的BPMN解析工场，该工厂负责创建bpmnParse对象
  ...
  public void initBehaviorFactory() {
    if (activityBehaviorFactory == null) { // 初始化 activityBehaviorFactory
  ...
  public void initBpmnParser() {
    if (bpmnParser == null) {
      bpmnParser = new BpmnParser(); // 实例化BpmnParser类
    }
    ...
    if (getPreBpmnParseHandlers() != null) {
      parseHandlers.addAll(getPreBpmnParseHandlers()); // 添加前置对象解析器
    }
    parseHandlers.addAll(getDefaultBpmnParseHandlers()); // 添加内置对象解析器
    if (getPostBpmnParseHandlers() != null) {
      parseHandlers.addAll(getPostBpmnParseHandlers()); // 添加后置对象解析器
    }
