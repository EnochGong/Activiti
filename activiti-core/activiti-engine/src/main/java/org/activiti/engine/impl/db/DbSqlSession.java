...
public class DbSqlSession implements Session {
    ...
    public void performSchemaOperationsProcessEngineBuild() {
        String databaseSchemaUpdate = Context.getProcessEngineConfiguration().getDatabaseSchemaUpdate();// 获取流程配置类中的DatabaseSchemaUpdate属性值
        log.debug("Executing performSchemaOperationsProcessEngineBuild with setting " + databaseSchemaUpdate);
        if (ProcessEngineConfigurationImpl.DB_SCHEMA_UPDATE_DROP_CREATE.equals(databaseSchemaUpdate)) { // 如果是这个属性drop-create
            try {
                dbSchemaDrop(); // 则就执行这个
